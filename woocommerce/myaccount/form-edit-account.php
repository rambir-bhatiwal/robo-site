...
<form class="woocommerce-EditAccountForm edit-account" action="" method="post" <?php do_action( 'woocommerce_edit_account_form_tag' ); ?> >
	...
	<p>
		...
		<button type="submit" class="btn btn-primary robo-btn woocommerce-Button button" name="save_account_details" value="<?php esc_attr_e( 'Save changes', 'woocommerce' ); ?>"><?php esc_html_e( 'Save changes', 'woocommerce' ); ?></button>
...
document.addEventListener('DOMContentLoaded', function() {
	var passwordFields = document.querySelectorAll('.edit-account input[type="password"]');
	passwordFields.forEach(function(input) {
		// Ensure we don't add duplicate toggles
		...
		wrapper.className = 'password-toggle-wrapper position-relative w-100';

		// Create toggle button
		var toggle = document.createElement('span');
		toggle.className = 'custom-show-password position-absolute';
		...
		toggle.innerHTML = '<i class="bi bi-eye text-muted fs-5"></i>';
		...
		toggle.addEventListener('click', function() {
			...
			if (input.type === 'password') {
				input.type = 'text';
				icon.className = 'bi bi-eye-slash text-muted fs-5';
			} else {
				input.type = 'password';
				icon.className = 'bi bi-eye text-muted fs-5';
			}
		});
	});
});
